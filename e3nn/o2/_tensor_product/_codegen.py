...
def codegen_tensor_product_left_right(
    ...
    for ins in instructions:
        ...
        if ins.connection_mode == "uvw":
            ...
            result = torch.einsum(f"{z}uvw,ijk,zuvij->zwk", w, d3m, xx)
        if ins.connection_mode == "uvu":
            assert mul_ir_in1.mul == mul_ir_out.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}uv,ijk,zuvij->zuk", w, d3m, xx)
            else:
                # not so useful operation because v is summed
                result = torch.einsum("ijk,zuvij->zuk", d3m, xx)
        if ins.connection_mode == "uvv":
            assert mul_ir_in2.mul == mul_ir_out.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}uv,ijk,zuvij->zvk", w, d3m, xx)
            else:
                # not so useful operation because u is summed
                # only specialize out for this path
                result = torch.einsum("ijk,zuvij->zvk", d3m, xx)
        if ins.connection_mode == "uuw":
            assert mul_ir_in1.mul == mul_ir_in2.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}uw,ijk,zuij->zwk", w, d3m, xx)
            else:
                # equivalent to tp(x, y, 'uuu').sum('u')
                assert mul_ir_out.mul == 1
                result = torch.einsum("ijk,zuij->zk", d3m, xx)
        if ins.connection_mode == "uuu":
            assert mul_ir_in1.mul == mul_ir_in2.mul == mul_ir_out.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}u,ijk,zuij->zuk", w, d3m, xx)
            else:
                result = torch.einsum("ijk,zuij->zuk", d3m, xx)
        if ins.connection_mode == "uvuv":
            assert mul_ir_in1.mul * mul_ir_in2.mul == mul_ir_out.mul
            if ins.has_weight:
                # TODO implement specialized code
                result = torch.einsum(f"{z}uv,ijk,zuvij->zuvk", w, d3m, xx)
            else:
                # TODO implement specialized code
                result = torch.einsum("ijk,zuvij->zuvk", d3m, xx)
        if ins.connection_mode == "uvu<v":
            ...
            if ins.has_weight:
                # TODO implement specialized code
                result = torch.einsum(f"{z}w,ijk,zwij->zwk", w, d3m, xx)
            else:
                # TODO implement specialized code
                result = torch.einsum("ijk,zwij->zwk", d3m, xx)
        if ins.connection_mode == "u<vw":
            ...
            # TODO implement specialized code
            result = torch.einsum(f"{z}qw,ijk,zqij->zwk", w, d3m, xx)
...
def codegen_tensor_product_right(
    ...
    for ins in instructions:
        ...
        # Create a proxy & request for the relevant wigner d3m
        # If not used (because of specialized code), will get removed later.
        d3m_name = f"_d3m_{mul_ir_in1.ir.l}_{mul_ir_in2.ir.l}_{mul_ir_out.ir.l}"
        d3m = fx.Proxy(graph.get_attr(d3m_name), tracer=tracer)

        if ins.connection_mode == "uvw":
            assert ins.has_weight
            result = torch.einsum(f"{z}uvw,ijk,zvj->zuiwk", w, d3m, x2)
        if ins.connection_mode == "uvu":
            assert mul_ir_in1.mul == mul_ir_out.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}uv,ijk,uw,zvj->zuiwk", w, d3m, e1, x2)
            else:
                # not so useful operation because v is summed
                result = torch.einsum("ijk,uw,zvj->zuiwk", d3m, e1, x2)
        if ins.connection_mode == "uvv":
            assert mul_ir_in2.mul == mul_ir_out.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}uv,ijk,zvj->zuivk", w, d3m, x2)
            else:
                # not so useful operation because u is summed
                # only specialize out for this path
                s2ones = fx.Proxy(
                ...
                result = torch.einsum("u,ijk,zvj->zuivk", s2ones, d3m, x2)
        if ins.connection_mode == "uuw":
            assert mul_ir_in1.mul == mul_ir_in2.mul
            if ins.has_weight:
                # TODO: specialize right()
                result = torch.einsum(f"{z}uw,ijk,zuj->zuiwk", w, d3m, x2)
            else:
                # equivalent to tp(x, y, 'uuu').sum('u')
                assert mul_ir_out.mul == 1
                result = torch.einsum("ijk,zuj->zuik", d3m, x2)
        if ins.connection_mode == "uuu":
            assert mul_ir_in1.mul == mul_ir_in2.mul == mul_ir_out.mul
            if ins.has_weight:
                result = torch.einsum(f"{z}u,ijk,uw,zuj->zuiwk", w, d3m, e1, x2)
            else:
                result = torch.einsum("ijk,uw,zuj->zuiwk", d3m, e1, x2)
        if ins.connection_mode == "uvuv":
            assert mul_ir_in1.mul * mul_ir_in2.mul == mul_ir_out.mul
            if ins.has_weight:
                # TODO implement specialized code
                result = torch.einsum(f"{z}uv,ijk,uw,zvj->zuiwvk", w, d3m, e1, x2)
            else:
                # TODO implement specialized code
                result = torch.einsum("ijk,uw,zvj->zuiwvk", d3m, e1, x2)
        if ins.connection_mode == "uvu<v":
