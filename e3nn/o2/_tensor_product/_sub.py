...
class ElementwiseTensorProduct(TensorProduct):
    ...
    filter_ir_out : iterator of `e3nn.o2.Irrep`, optional
        filter to select only specific `e3nn.o2.Irrep` of the output
    ...
    def __init__(self, irreps_in1, irreps_in2, filter_ir_out=None, irrep_normalization: str = None, **kwargs) -> None:
        irreps_in1 = o2.Irreps(irreps_in1).simplify()
        irreps_in2 = o2.Irreps(irreps_in2).simplify()
        if filter_ir_out is not None:
            try:
                filter_ir_out = [o2.Irrep(ir) for ir in filter_ir_out]
            except ValueError:
                raise ValueError(f"filter_ir_out (={filter_ir_out}) must be an iterable of e3nn.o2.Irrep")
...
class FullTensorProduct(TensorProduct):
    ...
    def __init__(
        ...
        filter_ir_out: Iterator[o2.Irrep] = None,
...
class TensorSquare(TensorProduct):
    ...
    irreps_out : `e3nn.o2.Irreps`, optional
        representation of the output
    ...
    def __init__(
        ...
        **kwargs,
        ...
        irreps_in = o2.Irreps(irreps_in).simplify()
        if filter_ir_out is not None:
            try:
                filter_ir_out = [o2.Irrep(ir) for ir in filter_ir_out]
        ...
        if irreps_out is None:
            ...
            irreps_out = o2.Irreps(irreps_out).simplify()
