...
class Linear(CodeGenMixin, torch.nn.Module):
    ...
    Notes
    -----
        `e3nn.o2.Linear` objects created with different partitionings of the same irreps, such as ``Linear("10x0e", "0e")``
        ...
        To make them equivalent, simplify ``irreps_in`` before constructing network modules:
            ...
            o2.Irreps("3x0e + 7x0e").simplify()  # => 10x0e
    ...
    internal_weights : bool
        whether the `e3nn.o2.Linear` should store its own weights. Defaults to ``True`` unless ``shared_weights`` is
        explicitly set to ``False``, for consistancy with `e3nn.o2.TensorProduct`.

    shared_weights : bool
        whether the `e3nn.o2.Linear` should be weighted individually for each input in a batch. Defaults to ``True``.
    ...
    ----------
    weight_numel : int
        the size of the weights for this `e3nn.o2.Linear`
