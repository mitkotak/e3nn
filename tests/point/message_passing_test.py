...
def test_equivariance(Rs_in, Rs_out, n_source, n_target, n_edge):
    ...
    groups = 4
    mp_groups = Convolution(Kernel(Rs_in, Rs_out, ConstantRadialModel), kernel_groups=groups) 
    ...
    out2_groups = mp_groups(features2 @ D_in_groups.T, edge_index, edge_r @ R.T, size=size) @ D_out_groups
...
def test_flow():
    ...
    for ker in conv.kernels:
        ker.R.weight.data.fill_(1.)  # Fix weight to 1.
