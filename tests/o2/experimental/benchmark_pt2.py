...
def main():
    for lmax in range(1, LMAX+1):
        ...
        tp = o2.FullTensorProduct(irreps_x, irreps_y) # Doesnt work with fullgraph=True

        
        tp_compile = torch.compile(tp, mode=compile_mode).to(device=device)
        print(f"TP lmax {lmax} channel {CHANNEL} batch {BATCH}: {print_performance(lambda: tp_compile(x, y), times=100, repeat=10)*1000:.3f}ms")
        
        tp_experimental = o2.experimental.FullTensorProductv2(irreps_x, irreps_y)
        tp_experimental_compile = torch.compile(tp_experimental, mode=compile_mode, fullgraph=True).to(device=device)
        print(f"TP Experimental lmax {lmax} channel {CHANNEL} batch {BATCH}: {print_performance(lambda: tp_experimental_compile(x, y), times=100, repeat=10)*1000:.3f}ms")
